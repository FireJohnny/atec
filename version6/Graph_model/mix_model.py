...
class model():
    ...
    def CNN_layer(self,variable_scope, x1, x2):
        with tf.variable_scope(variable_scope) as scope:
            ...
            L_conv = self.cnn_x1(self.pad_(x1), kernel_size=(self.emb_dim, self.filter_size),reuse = False)
    ...
    def f1score(self):
        real = tf.argmax(self.labels, 1)
        pred = tf.argmax(self.logits, 1)
        real_ones_like = tf.ones_like(real)
        real_zeros_like = tf.zeros_like(real)
        pred_ones_like = tf.ones_like(pred)
        pred_zeros_like = tf.zeros_like(pred)

        tp = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(real,real_ones_like),tf.equal(pred,pred_ones_like)),tf.float32))
        tn = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(real,real_zeros_like),tf.equal(pred,pred_zeros_like)),tf.float32))
        fp = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(real,real_zeros_like),tf.equal(pred,pred_ones_like)),tf.float32))
        fn = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(real,real_zeros_like),tf.equal(pred,pred_zeros_like)),tf.float32))

        tp_rate = tf.cast(tf.divide(tp,tf.add(tp,fn)),tf.float32)
        fp_rate = tf.cast(tf.divide(fp,tf.add(tp,fn)),tf.float32)

        acc = tf.cast(tf.divide(tf.add(tp,fn),tf.add(tf.add(tp,tn),tf.add(fp,fn))),tf.float32)
        recall = tp_rate
        precision = tf.divide(tp,tf.add(tp,fp))
        self.f1_score = tf.cast(tf.divide(tf.multiply(tf.constant(2,dtype=tf.float32),tf.multiply(recall,precision)),tf.add(recall,precision)),dtype=tf.float32)



    def merge(self):
        tf.summary.scalar(name = "loss", tensor=self.losses)
        tf.summary.scalar(name = "f1score", tensor=self.f1_score)
