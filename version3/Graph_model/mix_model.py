...
class model():
    ...
    def CNN_layer(self,variable_scope, x1, x2):
        with tf.variable_scope(variable_scope) as scope:
            ...
            R_conv = self.cnn_x2(self.pad_(x2), kernel_size=(self.emb_dim, self.filter_size),reuse = False)
    ...
    def LSTM_layer(self,variables_scope,x1,x2):
        with tf.variable_scope(variables_scope) as scope:
            ...
            R_att = self.Attention_2(R_rnn)
            with tf.variable_scope("att2att"):
                L_att_2 = self.Attention_1(x1_a)
                R_att_2 = self.Attention_2(x2_a)
    ...
    def build(self):
        ...
        pass
    def loss(self):
        return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.labels, logits=self.logits))
        pass
    def optimzer(self):
    ...
    def acc(self):
        self.acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.labels, 1), tf.argmax(self.logits, 1)), dtype=tf.float32))
